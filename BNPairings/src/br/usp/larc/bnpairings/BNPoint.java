...
public class BNPoint {
    ...
    /**
     * Multiples of the point by simple multiples of powers of 16.
     */
    protected BNPoint[][] pp16P;

    /**
     * The serialized point
     */
    byte[][][] pp16Pserial;//BNPointPersistable pp16Pserial;
    ...
    public BNPoint(BNCurve E, BigInteger x, int yBit) {
        ...
        if (x.signum() == 0) {
            ...
            this.y = E.bn.sqrt(x.multiply(x).multiply(x).add(E.getCurveCoefficient()).mod(p));
            if (y == null) {
                throw new IllegalArgumentException(pointNotOnCurve);
            }
...
/*
    public byte[][][] getSerializedTable() {
        ...
        if (pp16P == null) { //!pm.isPersisted(myPrecomputedTable))
            ...
            //setSerializedTable(myPrecomputedTable.pp16Pserial);
        }
        return pp16Pserial;
    }
//*/
...
//*/
    ...
    BNPoint(BNCurve E, int xTrit, BigInteger y) {
        ...
        if (y.signum() == 0) {
            ...
            this.x = E.bn.cbrt(y.multiply(y).subtract(E.getCurveCoefficient()).mod(p));
            if (x == null) {
                throw new IllegalArgumentException(pointNotOnCurve);
            }
    ...
    /**
    * Precompile scalar multiplication tables on demand, and serialize them.
    */
    public byte[][][] getSerializedTable() {
       if (pp16P == null) {
           pp16P = new BNPoint[(E.bn.n.bitLength() + 3)/4][16];
   ...
   /**
    * Load serialized tables into precompiled scalar multiplication tables
    */
    public void setSerializedTable(byte[][][] _pp16Pserial) {
       if (pp16P == null) {
           pp16P = new BNPoint[(E.bn.n.bitLength() + 3)/4][16];
    ...
    /**
    * Create a precompilable scalar multiplication table in the form
    of a String array with a given name
    */
    public void getCompilable(String name) {
       byte[][][] v = getSerializedTable();
       int N = (E.bn.n.bitLength() + 3)/4;
       int L = 2*((E.bn.p.bitLength() + 7)/8) + 1;
       char c = 0;
       char[] cc = new char[4];
               ...
               for (int k = 0; k < L; k++) {
                   ...
                   if ((m & 1) == 0) {
                       ...
                       case '\f':
                           ...
                           break;
                       default:
                           cc[0] = hex.charAt((c >>> 12)      );
                           cc[1] = hex.charAt((c >>>  8) & 0xf);
                           cc[2] = hex.charAt((c >>>  4) & 0xf);
                           cc[3] = hex.charAt((c       ) & 0xf);
                           System.out.print("\\u" + new String(cc));
                           break;
                       }
                   }
                   if ((m & 15) == 0) {
    ...
    /**
    * Load a precompiled String array into a scalar multiplication table
    */
    public void loadCompiledTable(String[] tab) {
       int N = (E.bn.n.bitLength() + 3)/4;
       int L = 2*((E.bn.p.bitLength() + 7)/8) + 1;
       byte[] os = new byte[L];
       if (pp16P == null) {
           pp16P = new BNPoint[N][16];
           for (int i = 0; i < N; i++) {
               String blk = tab[i];
               for (int j = 0; j < 16; j++) {
                   for (int k = 0; k < L; k++) {
                       int bpos = L*j + k;
                       char c = blk.charAt(bpos/2);
                       os[k] = (byte)((bpos & 1) == 0 ? c >> 8 : c & 0xff);
                   }
                   pp16P[i][j] = new BNPoint(E, os);
               }
           }
       /*
       } else {
           BNPoint[][] newtab = new BNPoint[N][16];
           for (int i = 0; i < N; i++) {
               String blk = tab[i];
               for (int j = 0; j < 16; j++) {
                   for (int k = 0; k < L; k++) {
                   ...
                   newtab[i][j] = new BNPoint(E, os);
                   if (!newtab[i][j].equals(pp16P[i][j])) {
    ...
    public BNPoint add(BNPoint Q) {
        ...
        BigInteger p = E.bn.p;
        BigInteger X1 = x, Y1 = y, Z1 = z, X2 = Q.x, Y2 = Q.y, Z2 = Q.z, Z1Z1 = BNParams._1, Z2Z2 = BNParams._1, U1 = x, U2 = Q.x, S1 = y, S2 = Q.y, H, I, J, R, V, X3, Y3, Z3;
    ...
    public BNPoint subtract(BNPoint Q) {
     ...
     /**Left-shift this point by a given distance n, i.e. compute (2^^n)*this </br>
     * </br> EFD doubling formulas: </br>
     * <a href="http://www.hyperelliptic.org/EFD/g1p/auto-shortw-jacobian-0.html#doubling-dbl-2009-l">
     * http://www.hyperelliptic.org/EFD/g1p/auto-shortw-jacobian-0.html#doubling-dbl-2009-l</a> </br>
     * <a href="http://www.hyperelliptic.org/EFD/g1p/auto-shortw-jacobian-0.html#doubling-mdbl-2007-bl">
      * http://www.hyperelliptic.org/EFD/g1p/auto-shortw-jacobian-0.html#doubling-mdbl-2007-bl</a>
     *
    ...
    public BNPoint multiply(BigInteger k) {
        if (pp16P == null) {
            ///System.out.println("Não-Precalculado");
            BNParams bn = E.bn;
            BNPoint P = this.normalize();
            if (k.signum() < 0) {
                k = k.negate();
            ...
            BigInteger t = bn.u.multiply(BNParams._3).add(BNParams._1).multiply(bn.u.shiftLeft(1)); // (3*u + 1)*2*u = 6*u^2 + 2*u
            //*/
            ...
            //System.out.println("----------------------------------------Precalculado");
            k = k.mod(E.bn.n);
            BNPoint A = E.infinity;
