...
int coap_handle_req(coap_rw_buffer_t *scratch, const coap_packet_t *inpkt, coap_packet_t *outpkt, const char *address, const char *request)
{
  ...
  while (NULL != ep->handler)
  {
    ...
    if (NULL != (opt = coap_findOptions(inpkt, COAP_OPTION_URI_PATH, &count)))
    {
      ...
      return ep->handler(scratch, inpkt, outpkt, inpkt->hdr.id[0], inpkt->hdr.id[1], address, request);
  }
next:
    ep++;
  }
...
/*Esta funcao talvez estivesse sendo chamada mais de uma vez
strcat resultava em path duplicado quando chamado por endpoints.c
sprintf nao duplica*/
...
void coap_get_path(coap_option_t opts[], uint8_t num_opts, char writebuf[]) {
  ...
  for (i = 0; i < num_opts; i++) {
    ...
    if (opts[i].num == 11) {
      coap_buffer_to_string(&temp, 300, &opts[i].buf);
      sprintf(writebuf, "%s", temp);
    }
  }
}
