...
abstract class BSClient extends BSEntity {
	...
	public final byte[] blind(byte[] commit) throws BSException {
		...
		result = BSUtil.adjustBigIntegerConversion(hLinhaA, securityLevel);
	...
	// Inicialmente protected, mas alterado para public para testes
	public byte[] encrypt(byte[] message, int len, byte[] receiverPublicKey)
			throws BSException {
		if (clientState != BSCLientState.READY)
			throw new BSException("Client not ready!");
		...
		return BSUtil.serialize(
				new byte[][]{
						c,
						BSUtil.adjustBigIntegerConversion(h, securityLevel),
	...
	public final byte[] signcrypt(byte[] message, int messageLenght,
		...
		cryptogram[0] = c;
		cryptogram[1] = BSUtil.adjustBigIntegerConversion(h, securityLevel);
		cryptogram[2] = BSUtil.adjustBigIntegerConversion(z, securityLevel);
	...
	private final byte[][] unsigncrypt(byte[] message, BNPoint Y_B)
		...
		byte[][] result = {m, BSUtil.adjustBigIntegerConversion(h, securityLevel), BSUtil.adjustBigIntegerConversion(z, securityLevel), sigma_tal[1]};
		return result;
	}
	...
	public byte[] getPrivateKey() throws BSException {
		...
		return BSUtil.adjustBigIntegerConversion(s_A, securityLevel);
	}
